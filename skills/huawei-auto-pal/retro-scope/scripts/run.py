...
def _extract_en_block(text: str, section_heading: str) -> str:
    """Extract the [EN] block under a ## heading from a PROPOSAL.md.

    Returns the English text, or empty string if not found.
    """
    import re as _re
    # Find the section: ## <heading> ... until next ## or end.
    pattern = _re.compile(
        r'^##\s+' + _re.escape(section_heading) + r'.*?\n(.*?)(?=^##\s|\Z)',
    ...
    if not m:
        return ""
    body = m.group(1)
    # Extract the [EN] block: [EN]\n... until [ZH] or end.
    en_match = _re.search(r'\[EN\]\s*\n(.*?)(?:\[ZH\]|\Z)', body, _re.DOTALL)
    if en_match:
        return en_match.group(1).strip()
    # No [EN] marker — return the whole section body (might be English-only).
    return body.strip()


def _extract_zh_block(text: str, section_heading: str) -> str:
    """Extract the [ZH] block under a ## heading from a PROPOSAL.md.

    Returns the Chinese text, or empty string if not found.
    """
    import re as _re
    pattern = _re.compile(
        r'^##\s+' + _re.escape(section_heading) + r'.*?\n(.*?)(?=^##\s|\Z)',
    ...
    if not m:
        return ""
    body = m.group(1)
    # Extract the [ZH] block: [ZH]\n... until next [EN] or end.
    zh_match = _re.search(r'\[ZH\]\s*\n(.*?)(?:\[EN\]|\Z)', body, _re.DOTALL)
    if zh_match:
        return zh_match.group(1).strip()
    # No [ZH] marker — fall back to English block (if any).
    return _extract_en_block(text, section_heading)


def _build_skills_section(output_dir: str, html_mod) -> str:
    """Build an HTML section summarizing proposed skills from output/.

    Reads each <skill-name>/PROPOSAL.md in output_dir, extracts the English
    Problem, Evidence, and Why-proposed sections, and renders them as cards.
    Returns an HTML string (empty if no skills found).
    """
    import os as _os

    # Find skill directories (have PROPOSAL.md, excluding session_records).
    skill_dirs = []
    try:
        for name in sorted(_os.listdir(output_dir)):
            full = _os.path.join(output_dir, name)
            if not _os.path.isdir(full):
                continue
            if name in ("session_records", "page_cache", "__pycache__",
                        ".skill-forge-backups", "reports"):
                continue
            proposal = _os.path.join(full, "PROPOSAL.md")
            if _os.path.isfile(proposal):
                skill_dirs.append((name, proposal))
    except OSError:
        return ""
    ...
    cards = []
    for name, proposal_path in skill_dirs:
        try:
        ...
        is_memory = name == "personal-context"
        skill_type = "记忆" if is_memory else "技能"
        type_badge = f'<span class="skill-type">{html_mod.escape(skill_type)}</span>'

        problem = _extract_zh_block(text, "Problem / 问题")
        evidence = _extract_zh_block(text, "Evidence / 证据")
        why = _extract_zh_block(text, "Why This Skill Is Proposed / 为什么提出这个技能")
        benefit = _extract_zh_block(text, "Benefit of Local Installation / 本地安装的收益")

        # Truncate long sections for the card (full text is in PROPOSAL.md).
        def _truncate_html(s: str, limit: int = 300) -> str:
            if len(s) <= limit:
                return s
            return s[:limit].rsplit(" ", 1)[0] + "…"

        problem_html = html_mod.escape(_truncate_html(problem))
        evidence_html = html_mod.escape(_truncate_html(evidence))
        why_html = html_mod.escape(_truncate_html(why))
        benefit_html = html_mod.escape(_truncate_html(benefit))

        fields = []
        if problem_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">问题：</span> {problem_html}</div>')
        if evidence_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">证据：</span> {evidence_html}</div>')
        if why_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">提出理由：</span> {why_html}</div>')
        if benefit_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">收益：</span> {benefit_html}</div>')

        fields_html = "\n".join(fields)
        cards.append(
            f'<div class="skill-card">\n'
            f'  <h3>{html_mod.escape(name)}{type_badge}</h3>\n'
            f'  {fields_html}\n'
            f'</div>'
        )
    ...
    cards_html = "\n".join(cards)
    return (
        f'<!-- SKILLS_SECTION_START -->\n'
        f'<div class="skills-section">\n'
        f'  <h2>建议的技能与记忆 ({len(cards)})</h2>\n'
        f'  <p class="hint">由 skill-forge 基于回溯分析结果生成。'
        f'详见各技能的 PROPOSAL.md。</p>\n'
        f'  {cards_html}\n'
        f'</div>\n'
        f'<!-- SKILLS_SECTION_END -->'
    )
...
def _run_multi_horizon(tasks: list[dict], events: list[dict],
    ...
    # Build the proposed-skills section by reading PROPOSAL.md files
    # from skill directories in output/.
...
<style>
  ...
  .skills-section {{ margin: 2em 0; }}
  .skills-section h2 {{ font-size: 1.3em; border-bottom: 2px solid #59a14f; padding-bottom: 0.2em; color: #2c2c2c; }}
  .skill-card {{ background: #f0f7f0; border: 1px solid #c8e6c8; border-radius: 8px; padding: 14px 18px; margin: 12px 0; }}
  .skill-card h3 {{ font-size: 1.1em; color: #2e7d32; margin: 0 0 0.3em 0; }}
  .skill-card .skill-type {{ font-size: 0.8em; color: #666; background: #e8f5e9; padding: 2px 8px; border-radius: 4px; margin-left: 8px; }}
  .skill-card .skill-field {{ font-size: 0.9em; margin: 0.4em 0; }}
  .skill-card .skill-field-label {{ font-weight: 600; color: #444; }}
</style>
</head>
<body>
