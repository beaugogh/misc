...
def _build_skills_section(output_dir: str, html_mod) -> str:
    ...
    for name, proposal_path in skill_dirs:
        ...
        skill_type = "memory" if is_memory else "skill"
        type_badge = f'<span class="skill-type">{html_mod.escape(skill_type)}</span>'

        problem = _extract_en_block(text, "Problem / 问题")
        evidence = _extract_en_block(text, "Evidence / 证据")
        why = _extract_en_block(text, "Why This Skill Is Proposed / 为什么提出这个技能")
        benefit = _extract_en_block(text, "Benefit of Local Installation / 本地安装的收益")
        ...
        fields = []
        if problem_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">Problem:</span> {problem_html}</div>')
        if evidence_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">Evidence:</span> {evidence_html}</div>')
        if why_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">Why proposed:</span> {why_html}</div>')
        if benefit_html:
            fields.append(f'<div class="skill-field"><span class="skill-field-label">Benefit:</span> {benefit_html}</div>')
    ...
    return (
        ...
        f'  <h2>Proposed skills &amp; memory ({len(cards)})</h2>\n'
        f'  <p class="hint">Generated by skill-forge from retro-scope findings. '
        f'See each skill\'s PROPOSAL.md for full details.</p>\n'
        f'  {cards_html}\n'
        f'</div>\n'
