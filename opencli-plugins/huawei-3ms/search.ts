...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const stats = await fetchStats(page, docs.map((d: any) => d.doc_id));
 ...
 */
async function fetchStats(page: any, docIds: string[]): Promise<Record<string, { views: string; likes: string; comments: string }>> {
