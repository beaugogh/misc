#!/usr/bin/env bash
# Set up the huawei-3ms OpenCLI plugin on this machine.
#
...
if out="$(opencli huawei-3ms search "大模型" --limit 1 2>&1)"; then
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "smoke test passed — plugin is ready"
  printf '\n%sDone.%s  Try:  opencli huawei-3ms search "架构演进"\n' "$GREEN" "$RESET"
else
  rc=$?
  printf '\n%sSmoke test failed — likely the Huawei session:%s\n' "$YELLOW" "$RESET"
  printf '%s\n' "$out" | sed 's/^/    /'
  printf '\n%s%sAction needed — a human must do this (an agent cannot):%s\n\n' "$BOLD" "$YELLOW" "$RESET"
  printf '  Open https://3ms.huawei.com/ in Chrome and sign in with your Huawei account,\n'
  printf '  then re-run:  ./setup.sh\n\n'
