 ...
 */
async function triggerSearch(page: any, query: string): Promise<boolean> {
    const ok = await page.evaluate((q: string) => {
        const input = document.querySelector('input.form-control') as HTMLInputElement | null
            || document.querySelector('input[placeholder*="搜"]') as HTMLInputElement | null;
        ...
        return new Promise<boolean>((resolve) => {
            setTimeout(() => {
                const btn = document.querySelector('.search-btn') as HTMLElement | null;
