...
# (not per-plugin — see opencli-plugins/README.md), and verifies with a real
# search.
#
# The NON-automatable parts (installing the Chrome Browser Bridge extension and
# signing into jx.huawei.com) must be done by a human first — see the
...
# Node's ESM resolver walks UP from the importing file, so the peer-dep does
# NOT need to live inside the plugin dir — it resolves from any ancestor
# node_modules. We hoist it to the REPO ROOT so the plugin folder stays clean
# (no per-plugin node_modules). `opencli plugin install` symlinks the plugin
# into ~/.opencli/plugins/, which points back here, so the root node_modules
# is still on the upward walk.
...
# it would shadow the root symlink and recreate the anti-pattern we fixed.
...
if out="$(opencli huawei-jiaxian search "大模型" --limit 1 2>&1)"; then
  ...
  printf '  Open https://jx.huawei.com/ in Chrome and sign in with your Huawei account,\n'
  printf '  then re-run:  ./setup.sh\n\n'
