#!/usr/bin/env bash
# Set up the huawei-chaspark OpenCLI plugin on this machine.
#
...
# signing into chaspark.com) must be done by a human first — see the
...
if out="$(opencli huawei-chaspark search "大模型" --limit 1 2>&1)"; then
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "smoke test passed — plugin is ready"
  printf '\n%sDone.%s  Try:  opencli huawei-chaspark search "光通信"\n' "$GREEN" "$RESET"
else
  rc=$?
  printf '\n%sSmoke test failed — likely the Huawei session:%s\n' "$YELLOW" "$RESET"
  printf '%s\n' "$out" | sed 's/^/    /'
  printf '\n%s%sAction needed — a human must do this (an agent cannot):%s\n\n' "$BOLD" "$YELLOW" "$RESET"
  printf '  Open https://www.chaspark.com/ in Chrome and sign in with your Huawei account,\n'
  printf '  then re-run:  ./setup.sh\n\n'
