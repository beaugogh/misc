...
A full catalog of every skill (across all four sources) **and every OpenCLI
plugin** lives in [`CATALOG.md`](./CATALOG.md) — generated by
...
[`CATALOG.md`](./CATALOG.md) — the single catalog of every skill and plugin in
this repo. Each plugin's `opencli-plugin.json` `commands` array is the source
of truth; regenerate with `./scripts/generate-catalog.sh`.
