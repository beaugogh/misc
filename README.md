...
tool** lives in [`CATALOG.md`](./CATALOG.md) — generated by
...
mcp-tools/
  <tool-name>/      # one MCP tool per folder — see mcp-tools/README.md
...
| [`huawei-auto-evolve`](./skills/huawei-auto-evolve) | Earlier self-evolution engine — analyzes sessions, WeLink, CodeHub to extract memory, create/update skills, recommend market skills. The operational machinery from this skill has been folded into huawei-auto-buddy's skill-forge component. |
...
MCP tool in this repo. Each plugin's `opencli-plugin.json` `commands` array is
...
adding a new tool. The tool table is generated into [`CATALOG.md`](./CATALOG.md);
each tool's `mcp-tool.json` is the source of truth.
