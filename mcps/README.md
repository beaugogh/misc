...
server is self-contained in its own folder under `mcps/`.

This is the repo's third artifact category, alongside [skills](../skills/) and
[OpenCLI plugins](../opencli-plugins/).

## Terminology: server vs tool

- **One folder = one MCP server** — a network endpoint (remote) or spawned
  process (local) that serves capabilities.
- **One server = one or many callable tools** — the functions it exposes over
  the MCP protocol. `huawei-w3-search` serves 1 tool; `huawei-wiki` 13;
  `huawei-clouddevops` 65. The folder name and `mcp-server.json` describe the
  *server*; its `tools[]` array documents the *tools*.

Nothing here assumes one folder = one tool — when adding a server with many
tools, that is the normal case, not a special one.

## Two transport types

Not all servers here are equal — the transport decides what "self-contained"
means:
...
(OpenCLI plugin). To make one "work for any agent," each server packages:
...
1. **Ready-to-load config** for each MCP-capable harness — Claude Code
   ...
   agent loads the matching file and the server's tools become available; no editing
...
mcps/
  <server-name>/
    mcp-server.json           # manifest — transport, url/command, env, tool args (catalog source of truth)
...
Pick one — see each server's `README.md` for specifics:

- **Any agent with Bash + Python 3** (no MCP support needed):
  ```bash
  python3 mcps/<server>/<wrapper>.py <args>
  ```
- **Claude Code**: `cp mcps/<server>/claude-code.mcp.json .mcp.json` (auto-discovered).
- **opencode / ngAgent / cac**: `codeagent --mcp-config mcps/<server>/opencode.mcp.json`.
...
See each server's `mcp-server.json` `network.no_proxy` field.
...
2. Add `mcp-server.json` (manifest: `name`, `transport` (`remote` or `local`),
...
See [`../CATALOG.md`](../CATALOG.md) → "MCP servers" section (generated).
